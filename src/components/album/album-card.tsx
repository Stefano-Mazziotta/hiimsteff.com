...
export default async function AlbumCard({ album }: { album: Album }) {
  ...
  return (
    <Link
      href={`/gallery/${slug}`}
      className="h-100 group relative isolate z-10 flex min-h-60 w-full flex-col justify-start border-4 border-white"
    >
      <h2 className="z-20 mb-2 ml-2 mt-2 inline-block w-fit bg-black bg-opacity-100 px-2 py-1 text-base font-semibold text-white">
        {title}
      </h2>
      <Image
        src={featuredPhoto}
